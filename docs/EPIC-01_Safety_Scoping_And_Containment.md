...
| Component | Status |
|---|---|
| `SECURITY.md` — what to report, and what not to | Planned |
| README safety banner (replaces EPIC-00 marker) | Planned |
| Containment boundary — isolated deploy target | Planned |
| Synthetic-data-only policy for the two DB tables | Planned |
| Owned npm namespace (`@mist-demo/*`) registered | Planned |
| Slopsquat placeholder policy | Planned |
| `scripts/check-containment.sh` + CI gate | Planned |
| Key-rotation runbook for the weather API key | Planned |
...
|---|---|---|
| The Containment Boundary | isolated cloud project + `deploy/isolation.md` | ❌ absent |
| The Disclaimer | README banner + `SECURITY.md` | ❌ absent |
| Owned Namespace | npm scope `@mist-demo` | ❌ absent |
| Slopsquat Placeholder | `@mist-demo/<hallucinated-name>` inert package | ❌ absent |
| Burnt Credential (the honestly-committed key) | git history + `docs/KEY_ROTATION.md` | ❌ absent |
| Synthetic Subject (fake user rows) | seed script, EPIC-02 | ❌ absent |
